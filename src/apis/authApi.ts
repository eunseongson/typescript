...
import { IClientCredentialTokenbResponse, IExchangedTokenResponse } from "../models/auth";
...
export const exchangetToken = async (code: string, codeVerifier: string): Promise<IExchangedTokenResponse> => {
    try {
        const url = "https://accounts.spotify.com/api/token";
        if (!CLIENT_ID || !REDIRECT_URI) {
            throw new Error('CLIENT_ID or REDIRECT_URI is missing');
        }
        const body = new URLSearchParams({
            client_id: CLIENT_ID,
